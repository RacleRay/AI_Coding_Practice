...
class TextCNN(object):
    ...
    def __init__(self,
        ...
        with tf.variable_scope('Embedding'):
            embed = tf.contrib.layers.embed_sequence(self.input_x,
        ...
        for i, filter_size in enumerate(filter_sizes):
            conv = slim.conv2d(self.embedded_chars_expanded,
                               num_outputs=num_filters,
                               kernel_size=[filter_size, embedding_size],
                               stride=1,
                               padding="VALID",
                               activation_fn=tf.nn.leaky_relu,
                               scope="conv%s" % filter_size)
            pooled = slim.max_pool2d(conv,
        ...
        with tf.name_scope("output"):
            self.scores = slim.fully_connected(self.h_drop,
                                               num_classes,
                                               activation_fn=None,
