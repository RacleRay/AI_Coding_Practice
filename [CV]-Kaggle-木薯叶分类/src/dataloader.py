import torch
import numpy as np
from fmix import make_low_freq_image, binarise_mask, sample_mask
    ...
    def __init__(self, df, data_root,
        ...
        Args:
            ...
            fmix_params :dict , fmix 的参数 {'alpha':1.,'decay_power':3.,'shape':(256,256),'max_soft':0.3,'reformulate':False}
                                decay_power：公式中的次幂，可以理解为对高频的惩罚程度
...
def prepare_dataloader(data_aug_param, df, trn_idx, val_idx, data_root):
    '''多进程数据生成器
    Args:
        data_aug_param: nni调参传入的数据增强参数
        df : DataFrame , 样本图片的文件名和标签
    ...
    train_ds = CassavaDataset(train_, data_root,
                              transforms=get_train_transforms(data_aug_param),
                              output_label=True,
                              one_hot_label=False,
                              do_fmix=False,
                              do_cutmix=False)
